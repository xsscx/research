...
```bash
# All 13 fuzzers on ramdisk (default)
cd cfl && ./fuzz-local.sh

# All 13 fuzzers on SSD
...
- **Binary-to-corpus mapping**: Only 13 fuzzers have binaries. 8 orphaned corpus dirs deleted. `corpus-xml` (48 named XML seeds) kept as staging area for `icc_fromxml_fuzzer`.
