...
if $DRY_RUN; then
  echo "🔍 DRY RUN — showing what would be removed (pass --execute to apply)"
else
  echo "🗑  EXECUTING — removing stray items from $RAMDISK"
fi
echo ""
...
for d in "$RAMDISK"/corpus-*; do
  ...
  if ! is_legit_corpus "$name"; then
    ...
    if $DRY_RUN; then
      ...
      rm -rf "$d"
      echo "  ✓ removed $name  ($count files, $size_h)"
    fi
