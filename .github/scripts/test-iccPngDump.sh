...
for png_file in "$REPO_ROOT"/fuzz/graphics/png/CVE-*.png \
  ...
  [ -f "$png_file" ] || continue
  file -b -- "$png_file" 2>/dev/null | grep -q '^PNG image data' || continue
  CVE_PNGS+=("$png_file")
done
...
if [ -d "$FUZZ_PNG_DIR" ]; then
  ...
  while IFS= read -r f; do
    file -b -- "$f" 2>/dev/null | grep -q '^PNG image data' || continue
    FUZZ_PNGS+=("$f")
  done < <(find "$FUZZ_PNG_DIR" -maxdepth 1 -name '*.png' 2>/dev/null | shuf -n "$MAX_BATCH")
