...
# Encoding variations with sRGB (0=Value, 1=Percent, 2=UnitFloat, 3=Float,
# 4=8Bit, 5=16Bit, 6=16BitV2). The single profile ends in XYZ, which does
# not support 8-bit output, so use a second sRGB profile to end in RGB.
...
run_test "ncm-05" "sRGB chain encoding=4 (8Bit)" "$APPLYNCM" "$SRGB_CALC_DATA" 4 0 "$SRGB" 1 "$SRGB" 1
run_test "ncm-06" "sRGB encoding=5 (16Bit)" "$APPLYNCM" "$SRGB_CALC_DATA" 5 0 "$SRGB" 1
run_test "ncm-06-v2" "sRGB encoding=6 (16BitV2)" "$APPLYNCM" "$SRGB_CALC_DATA" 6 0 "$SRGB" 1
...
# Config export/replay and calculator-debug option placement
EXPORTED_CFG="$OUTDIR/ncm-export-with-data.json"
run_test "ncm-17" "Export config with embedded data" \
  "$APPLYNCM" -exportcfganddata "$EXPORTED_CFG" "$SRGB_CALC_DATA" 0 0 "$SRGB" 1
if [ -s "$EXPORTED_CFG" ]; then
  run_test "ncm-18" "Replay exported config with -cfg" "$APPLYNCM" -cfg "$EXPORTED_CFG"
else
  TOTAL=$((TOTAL + 1))
  FAIL=$((FAIL + 1))
  printf "  [%-7s] %-55s exit=%-3d%s\n" "FAIL" "Replay exported config with -cfg" 1 " [missing export]"
fi
