...
if $DO_MERGE; then
  ...
  "$SCRIPT_DIR/ramdisk-merge.sh" || echo "  ⚠ Merge had failures (continuing with sync)"
  echo ""
fi
...
if $DO_UNMOUNT; then
  ...
  if mountpoint -q "$RAMDISK" 2>/dev/null; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "  ⚠ Not root — cannot unmount. Run with sudo or use --no-unmount."
    else
      umount "$RAMDISK" 2>/dev/null || echo "  ⚠ umount failed (ramdisk may be busy)"
      rmdir "$RAMDISK" 2>/dev/null || true
      echo "  ✓ Ramdisk unmounted and removed"
    fi
  else
