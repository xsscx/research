...
- `iccanalyzer-lite/iccanalyzer-lite` — 173-heuristic security analyzer (ASAN+UBSAN)
...
─────────────────                     ──────────────────────────────
  ...
  curl -F file=@profile.icc  ───────→  ghcr.io/xsscx/icc-profile-mcp web
    POST /api/upload                    │
                                        ├── iccanalyzer-lite -a (173 heuristics)
...
| `/api/security?path=...` | GET | 173-heuristic scan (text) |
| `/api/security-json?path=...` | GET | 173-heuristic scan (structured JSON) |
...
# → Full 173-heuristic JSON analysis
...
**Analyzed**: 49 profiles/images with full 173-heuristic reports in `analysis-reports/`
