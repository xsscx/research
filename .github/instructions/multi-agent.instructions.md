...
- `iccanalyzer-lite/iccanalyzer-lite` — 180-heuristic security analyzer (ASAN+UBSAN)
...
─────────────────                     ──────────────────────────────
  ...
  curl -F file=@profile.icc  ───────→  ghcr.io/xsscx/icc-profile-mcp web
    POST /api/upload                    │
                                        ├── iccanalyzer-lite -a (180 heuristics)
...
| `/api/security?path=...` | GET | 180-heuristic scan (text) |
| `/api/security-json?path=...` | GET | 180-heuristic scan (structured JSON) |
...
# → Full 180-heuristic JSON analysis
...
**Analyzed**: 49 profiles/images with full 180-heuristic reports in `analysis-reports/`
