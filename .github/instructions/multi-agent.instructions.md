...
─────────────────                     ──────────────────────────────
  ...
  curl -F file=@profile.icc  ───────→  ghcr.io/xsscx/icc-profile-mcp web
    POST /api/upload                    │
                                        ├── iccanalyzer-lite -a (159 heuristics)
...
| `/api/security?path=...` | GET | 159-heuristic scan (text) |
| `/api/security-json?path=...` | GET | 159-heuristic scan (structured JSON) |
...
# → Full 159-heuristic JSON analysis
...
**Analyzed**: 49 profiles/images with full 159-heuristic reports in `analysis-reports/`
