...
─────────────────                     ──────────────────────────────
  ...
  curl -F file=@profile.icc  ───────→  ghcr.io/xsscx/icc-profile-mcp web
    POST /api/upload                    │
                                        ├── iccanalyzer-lite -a (153 heuristics)
...
| `/api/security?path=...` | GET | 153-heuristic scan (text) |
| `/api/security-json?path=...` | GET | 153-heuristic scan (structured JSON) |
...
# → Full 153-heuristic JSON analysis
