...
- `iccanalyzer-lite/iccanalyzer-lite` — 171-heuristic security analyzer (ASAN+UBSAN)
...
─────────────────                     ──────────────────────────────
  ...
  curl -F file=@profile.icc  ───────→  ghcr.io/xsscx/icc-profile-mcp web
    POST /api/upload                    │
                                        ├── iccanalyzer-lite -a (171 heuristics)
...
| `/api/security?path=...` | GET | 171-heuristic scan (text) |
| `/api/security-json?path=...` | GET | 171-heuristic scan (structured JSON) |
...
# → Full 171-heuristic JSON analysis
...
**Analyzed**: 49 profiles/images with full 171-heuristic reports in `analysis-reports/`
