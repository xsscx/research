...
| 21 | If a second automated review finds any new blocker, including one in the repair, set `review-stop: FAIL - maintainer direction required`. Do not launch a local or cloud reviewer, push another repair, resolve findings as closure, or claim readiness; return to branch-only grooming for the cumulative audit. | Repeated review cycles conceal inadequate pre-PR preparation and waste reviewer capacity. |
