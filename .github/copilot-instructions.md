...
# Build CFL fuzzers (clones iccDEV, applies 55 patches, builds 19 fuzzers)
cd cfl && ./build.sh
...
- **cfl/patches/** — 55 active security patches applied to iccDEV before fuzzer builds. Includes OOM caps (16MB–128MB), UBSAN fixes, heap-buffer-overflow guards, stack-overflow depth caps, null-deref guards, memory leak fixes, float-to-int overflow clamps, alloc/dealloc mismatch corrections, recursion depth limits, IO underflow guards, calculator ops array bounds clamping, XML entity expansion caps, XML parsing limits, and CLUT interpolation bounds checks (Interp1d–6d + InterpND). 14 patches dropped as NO-OPs during upstream sync (023, 027, 028, 029, 032, 039, 040, 041, 045, 055, 056, 058, 062, 066). Next patch: 070. See `cfl/patches/README.md` for full details.
