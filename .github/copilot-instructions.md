...
When an issue asks to analyze an ICC profile, you MUST:
1. Use the `analyze_security` MCP tool (or run `iccanalyzer-lite/iccanalyzer-lite -a <file>`) and include the **complete raw output** in the report
2. Use the `inspect_profile` MCP tool (or run `iccanalyzer-lite/iccanalyzer-lite -n <file>`) and include the **complete raw output** in the report
3. Use the `validate_roundtrip` MCP tool and include its output
4. Include the iccanalyzer-lite exit code and any ASAN/UBSAN stderr output
5. Add your own analysis AFTER the raw tool output — do not replace it with a summary

The raw iccanalyzer-lite output uses `[OK]`, `[WARN]`, `[FAIL]`, `[CRITICAL]` labels and heuristic IDs `[H1]`–`[H19]`. This output MUST appear verbatim in the report.
