...
| **OOM Patches** | 41 patches | Security fixes in cfl/patches/ |
...
The `cfl/patches/` directory contains 41 security patches for iccDEV (OOM caps, OOB reads, UBSAN fixes, null-deref guards, heap-buffer-overflow fixes). Applied automatically by `cfl/build.sh`. See `cfl/patches/README.md` for the full catalog.
