...
| **OOM Patches** | 51 patches | Security fixes in cfl/patches/ |
...
The `cfl/patches/` directory contains 51 security patches for iccDEV (OOM caps, OOB reads, UBSAN fixes, null-deref guards, heap-buffer-overflow fixes). Applied automatically by `cfl/build.sh`. See `cfl/patches/README.md` for the full catalog.
