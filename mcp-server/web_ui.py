...
_ALLOWED_DIRS = frozenset({"test-profiles", "extended-test-profiles", "xif", "fuzz/graphics/icc", "reference-profiles"})
...
async def api_xml_download(request: Request) -> Response:
    ...
    try:
        ...
        async with (await _get_semaphore()):
            ...
            # Check for pre-generated XML companion (reference-profiles)
            ...
            if companion_xml.is_file():
                try:
                    ...
                    fsize = companion_xml.stat().st_size
                    if fsize > MAX_DOWNLOAD_BYTES:
                        return JSONResponse(
                            {"ok": False, "error": f"XML file too large ({fsize:,} bytes)"},
                            status_code=400,
                        )
                    content = companion_xml.read_text(errors="replace")
