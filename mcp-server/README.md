...
**Your AI assistant calls** `analyze_security("cve-2022-26730-poc-sample-004.icc")` and returns a 14-phase heuristic report covering header validation, tag anomalies, overflow indicators, and malicious patterns.
...
| Button | What it does |
|--------|-------------|
| **List Profiles** | Browse available ICC profiles by directory (`test-profiles`, `extended-test-profiles`, `reference-profiles`) |
| **Inspect** | Dump raw header bytes, tag table, and parsed field values |
| **Security Scan** | Run 14-phase heuristic security analysis |
...
**What it does:** Runs a 14-phase security heuristic scan checking for fingerprint matches, tag anomalies, overflow indicators, malformed signatures, and known attack patterns.
