...
def test_list():
    ...
    for d_name in ["test-profiles", "extended-test-profiles", "reference-profiles", "xif"]:
...
def test_xml():
    r = c.get("/api/xml?path=reference-profiles/Display_sRGB_D65_colorimetric.icc")
...
def test_xml_download():
    r = c.get("/api/xml/download?path=reference-profiles/Display_sRGB_D65_colorimetric.icc")
