...
REFERENCE_PROFILES = REPO_ROOT / "reference-profiles"

# Allowed base directories for profile resolution (resolved at import time)
_ALLOWED_BASES = [REPO_ROOT, TEST_PROFILES, EXTENDED_PROFILES, XIF_DIR, FUZZ_ICC_DIR, REFERENCE_PROFILES]
...
def _resolve_profile(path: str) -> Path:
    ...
    raise FileNotFoundError(
        ...
        f"xif/, fuzz/graphics/icc/, reference-profiles/"
    )
...
async def analyze_security(path: str) -> str:
    """Run 14-phase security heuristic analysis on an ICC profile.
...
async def profile_to_xml(path: str) -> str:
    ...
    For reference-profiles, serves pre-generated XML directly (instant).
    Otherwise tries the safe iccToXml first; falls back to iccToXml_unsafe
    ...
    # Check for pre-generated XML companion file (reference-profiles)
    companion_xml = profile.with_suffix(".xml")
    if companion_xml.is_file():
        try:
            companion_xml.resolve().relative_to(REFERENCE_PROFILES.resolve())
            file_size = companion_xml.stat().st_size
            with open(companion_xml, "r", errors="replace") as fh:
                content = fh.read(max_xml_chars + 1)
            content = _sanitize_output(content)
            header = "[Pre-generated XML from reference-profiles]\n\n"
            ...
            pass  # Not in reference-profiles or read error — fall through
...
async def list_test_profiles(directory: str = "test-profiles") -> str:
    ...
    Args:
        directory: Which profile set — "test-profiles", "extended-test-profiles",
                   "xif", "fuzz/graphics/icc", or "reference-profiles"
    """
    dirs = {
