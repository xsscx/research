...
def _resolve_profile(path: str) -> Path:
    ...
    for base, base_resolved in zip(_ALLOWED_BASES, _ALLOWED_BASES_RESOLVED):
        ...
        try:
            ...
            continue
        if candidate.is_file():  # CodeQL[py/path-injection]: path validated via resolve()+relative_to() above
            return candidate
...
def _require_binary(bin_path: Path, name: str) -> None:
    if not bin_path.exists():  # CodeQL[py/path-injection]: bin_path is a compile-time constant (REPO_ROOT/...)
        raise FileNotFoundError(
