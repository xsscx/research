...
constexpr std::array<PawgItemDef, 13> kSecurityItems{{
    {"S1", "Do channel counts in tags match the data colour space?", {60, 61}, PawgVerdict::GAP},
    {"S2", "Is header 128 bytes and correctly encoded?", {10, 6, 12, 13}, PawgVerdict::NOT_APPLICABLE},
    {"S3", "Do Platform, Creator, Manufacturer and CMM fields correspond to registered signatures or are zero?", {7}, PawgVerdict::NOT_APPLICABLE},
    {"S4", "Does illuminant correspond to D50?", {8}, PawgVerdict::NOT_APPLICABLE},
    {"S5", "Is PCS Lab or XYZ (unless DeviceLink profile)?", {14}, PawgVerdict::NOT_APPLICABLE},
    {"S6", "Are tags correctly aligned - offset and length correspond to tag table, no overlapping tags or gaps between tags - and correctly encoded?", {20, 103, 107}, PawgVerdict::NOT_APPLICABLE},
    {"S7", "Is the tag table correctly encoded?", {40}, PawgVerdict::NOT_APPLICABLE},
    {"S8", "Is the profile free of known malware signatures?", {91}, PawgVerdict::NOT_APPLICABLE},
    {"S9", "Does EOF follow last tag (including four-byte boundary), with no additional bytes before or after?", {10}, PawgVerdict::NOT_APPLICABLE},
    {"S10", "[iccMAX profiles only] Are excessive calculator elements avoided (if possible provide an estimate of computation cost)", {88, 62}, PawgVerdict::NOT_APPLICABLE},
    {"S11", "Are private tags absent?", {92}, PawgVerdict::NOT_APPLICABLE},
    {"S12", "If present, are private tags free of malware?", {93}, PawgVerdict::NOT_APPLICABLE},
    {"S13", "If present, are private tags free of exploitable non-operation (NOP) instructions?", {94}, PawgVerdict::NOT_APPLICABLE},
}};

constexpr std::array<PawgItemDef, 14> kConformanceItems{{
    {"C1", "Are tag types correctly encoded (signature, structure, data types, ranges, encoded values)?", {20, 22, 23, 24, 25, 26, 27, 28, 29, 32, 33, 34, 112}, PawgVerdict::NOT_APPLICABLE},
    {"C2", "Are cprt and desc tags encoded as Unicode or text according to specification version?", {40}, PawgVerdict::NOT_APPLICABLE},
    {"C3", "Do tags only use tag types allowed for the tag?", {20}, PawgVerdict::NOT_APPLICABLE},
    {"C4", "Are all required tags for profile class present?", {40, 41, 42, 43, 44, 45, 46, 47, 104, 111}, PawgVerdict::NOT_APPLICABLE},
    {"C5", "Is the profile free of additional tags not required for profile class (other than allowed optional tags); if present are they flagged as private tags?", {95}, PawgVerdict::NOT_APPLICABLE},
    {"C6", "If present, do private tags have a registered signature?", {96}, PawgVerdict::NOT_APPLICABLE},
    {"C7", "If present, is documentation for private tags available through the tag registry?", {97}, PawgVerdict::NOT_APPLICABLE},
    {"C8", "Are any undocumented private tags identified?", {98}, PawgVerdict::NOT_APPLICABLE},
    {"C9", "Is the profile class consistent with the data colour space?", {12, 13}, PawgVerdict::NOT_APPLICABLE},
    {"C10", "Does the header content conform with the specification?", {1, 2, 3, 4, 5, 6, 8, 9, 14, 15, 121, 122}, PawgVerdict::NOT_APPLICABLE},
    {"C11", "Do the tags present correspond to the profile version?", {48, 53}, PawgVerdict::NOT_APPLICABLE},
    {"C12", "Is the white point correctly encoded (D50 for v4 display; or valid value for other profile classes)?", {8}, PawgVerdict::NOT_APPLICABLE},
    {"C13", "Are reserved bytes zero?", {15}, PawgVerdict::NOT_APPLICABLE},
    {"C14", "Do tags start and end on four-byte boundaries?", {20, 103}, PawgVerdict::NOT_APPLICABLE},
}};
...
constexpr std::array<PawgItemDef, 4> kQualityItems{{
    ...
    {"Q4", "Round trip average and maximum differences of profile output in CIEDE2000 (if characterization data is present)", {102}, PawgVerdict::GAP},
}};
