...
int RunHeuristic_H142_XmlSerializationSafety(CIccProfile * /*pIcc*/, const char *filename)
{
  ...
  hc.begin(142, "XML Serialization Safety (Section 10 Tag Type Definitions)");
...
int RunHeuristic_H180_XmlRoundTripFidelity(CIccProfile * /*pIcc*/, const char *filename)
{
  ...
  if (pid == 0) {
    ...
    if (tmpFd < 0)
      _exit(0);
    bool wroteAll = iccxml::WriteAllBytes(tmpFd, xmlOutput.c_str(), xmlOutput.size());
    close(tmpFd);
    if (!wroteAll) {
      unlink(tmpXml);
      _exit(0);
    ...
    // Keep the round-trip parser in local-only mode even for self-generated XML.
