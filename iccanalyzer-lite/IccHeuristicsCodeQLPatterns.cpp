...
// Historical iccDEV IccUtil.cpp signature-formatting helpers used to iterate
// FourCC signatures with "sig <<= 8", which triggered UBSAN for normal
// non-zero first bytes. Upstream iccDEV fixed this in #726, and the analyzer
// links safe overrides for older static libraries, so profile content alone is
// no longer evidence of runtime UB.
// ---------------------------------------------------------------------------
int RunHeuristic_H173_SigConversionShiftOverflow(RawProfileContext &ctx)
{
  ...
  return hc.skip("Resolved upstream: iccDEV #726 removed the sig <<= 8 "
                 "signature helper pattern; analyzer safe overrides are active");
}
...
// Historical iccDEV icF16toF() used unsigned exponent rebias arithmetic for
// half-float values below 1.0. Upstream fixed the conversion in #724/#727 by
// using signed temporary exponent arithmetic, so these raw half-float values
// are no longer evidence of runtime UB.
// ---------------------------------------------------------------------------
int RunHeuristic_H174_HalfFloatConversionUnsignedUnderflow(RawProfileContext &ctx)
{
  ...
  if (!ctx.valid) return hc.skip("Cannot read profile");
  return hc.skip("Resolved upstream: iccDEV #724/#727 fixed icF16toF() "
                 "half-float exponent rebias arithmetic");
}
