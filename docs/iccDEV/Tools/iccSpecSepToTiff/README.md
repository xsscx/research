...
```
iccSpecSepToTiff output compress sep infile_prefix start end incr {profile}
```
...
| `infile_prefix` | **Required** | Prefix for input files; channel numbers are appended with no suffix after the number (e.g., `spec_00` with start `1` opens `spec_001`) |
...
Given spectral separation TIFFs named `spec_380` through `spec_780`
at 5nm intervals:
...
Individual spectral TIFFs          Multi-channel TIFF
  spec_380
  spec_385  ---> iccSpecSepToTiff ---> spectral_merged.tiff (81 channels)
  spec_390
  ...
  spec_780
```
...
```bash
# 4x4 8-bit baseline series (10 channels)
...
- Requires sequential TIFF files whose names match the prefix plus channel number exactly
...
- The spectral TIFF at `iccDEV/Testing/hybrid/Data/smCows380_5_780.tif` has
  81 SPP - it cannot be used as input (it is the expected output format)
...
| Basic merging (10-channel, 81-channel, 8x8, 16-bit) | 6 | All PASS |
...
All combinations of `compress x sep x profile x seed_series`:
